...
v2ray_config() {
        ...
        echo
        while :; do
                echo -e "请选择 "$yellow"V2Ray"$none" 传输协议 [${magenta}1-${#transport[*]}$none]"                echo
...
v2ray_dynamic_port_start() {
        ...
        while :; do
                echo -e "请输入 "$yellow"V2Ray 动态端口开始 "$none"范围 ["$magenta"1-65535"$none"]"                read -p "$(echo -e "(默认开始端口: ${cyan}10000$none):")" v2ray_dynamic_port_start_input
...
v2ray_dynamic_port_end() {
        ...
        while :; do
                echo -e "请输入 "$yellow"V2Ray 动态端口结束 "$none"范围 ["$magenta"1-65535"$none"]"                read -p "$(echo -e "(默认结束端口: ${cyan}20000$none):")" v2ray_dynamic_port_end_input
...
get_ip() {
        ...
        [[ -z $ip ]] && ip=$(curl -s https://api.myip.com | grep -oE "([0-9]{1,3}\.){3}[0-9]{1,3}")        [[ -z $ip ]] && ip=$(curl -s icanhazip.com)
...
uninstall() {
        ...
        if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
                ...
                if [[ $mark ]]; then
                        ...
                        echo
                        echo -e " $yellow输入 ${cyan}v2ray uninstall${none} $yellow即可卸载${none}"                        echo
                fi
...
root@FatLightskyblue-VM:~# ls
hh.txt  mysql-apt-config_0.8.13-1_all.deb  ssr.sh  v2ray.sh  virt-sysprep-firstboot.log
root@FatLightskyblue-VM:~# cat v2ray.sh 
#!/bin/bash
